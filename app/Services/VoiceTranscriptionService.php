...
class VoiceTranscriptionService
{
    ...
    public function transcribe(string $mime, string $bytes, string $locale, array $context = []): array
    {
        ...
        $interaction=AiInteraction::create(['user_id'=>$context['user_id']??null,'provider'=>'gemini','model'=>$model,'operation'=>'voice_transcription','request_id'=>$requestId,'prompt_hash'=>hash('sha256',$prompt),'input_bytes'=>(int)($context['input_bytes']??strlen($bytes)),'status'=>'started','input_meta'=>['mime'=>$mime,'locale'=>$locale]]);
        $started=hrtime(true);
        try {
            $response=Http::timeout(120)->retry(2,650)->withHeaders(['x-goog-api-key'=>$key,'Content-Type'=>'application/json','Api-Revision'=>'2026-05-20','X-Farast-Request-Id'=>$requestId])->post(self::ENDPOINT,['model'=>$model,'input'=>[['type'=>'text','text'=>$prompt],['type'=>'audio','data'=>base64_encode($bytes),'mime_type'=>$mime]],'store'=>false,'system_instruction'=>'Faithful transcription only. Never invent content.']);
            $latency=(int)((hrtime(true)-$started)/1000000);
            if(!$response->successful()) throw new \RuntimeException('Gemini voice HTTP '.$response->status().' '.$response->body());
            $text=trim((string)$response->json('output_text',''));
            if($text==='') $text=collect($response->json('outputs',[]))->filter(fn($output)=>($output['type']??null)==='text')->pluck('text')->implode('');
            if($text==='') $text=collect($response->json('steps',[]))->flatMap(fn($step)=>$step['content']??[])->filter(fn($content)=>($content['type']??null)==='text')->pluck('text')->implode('');
            $text=trim($text);
            if($text==='') throw new \RuntimeException('Gemini returned an empty voice transcript.');
            ...
            return ['text'=>$text,'interaction_id'=>$interaction->id,'request_id'=>$requestId];
        } catch(\Throwable $e) {
            $interaction->update(['latency_ms'=>(int)((hrtime(true)-$started)/1000000),'status'=>'failed','error_message'=>mb_substr($e->getMessage(),0,4000)]);
            throw $e;
        }
    }
}
