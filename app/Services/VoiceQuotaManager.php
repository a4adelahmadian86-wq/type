...
class VoiceQuotaManager
{
    ...
    public function availableSeconds(VoiceProviderAccount $account): ?int
    {
        ...
        if ($account->billing_mode === 'trial') {
            ...
            if ($account->credit_remaining !== null) {
                return max(0, (int) floor((float) $account->credit_remaining));
            }
        }

        return $account->quota_limit_seconds === null
            ? null
            : max(0, (int) $account->quota_limit_seconds - (int) $account->quota_used_seconds);
    }
    ...
    public function canStart(VoiceProviderAccount $account, int $minimumSeconds = 1): bool
    {
        ...
        return $available >= ($minimumSeconds + self::SAFETY_RESERVE_SECONDS);
    }
    ...
    public function consume(VoiceProviderAccount $account, float $audioSeconds, int $inputBytes = 0, int $outputBytes = 0, bool $success = true): void
    {
        ...
        DB::transaction(function () use ($account, $seconds, $inputBytes, $outputBytes, $success) {
            $row = VoiceProviderAccount::query()->lockForUpdate()->find($account->id);
            if (!$row) return;

            $this->resetMonthlyPeriodIfNeeded($row);

            $row->quota_used_seconds = (float) $row->quota_used_seconds + $seconds;
            if ($row->billing_mode === 'trial' && $row->credit_remaining !== null) {
                $row->credit_remaining = max(0, (float) $row->credit_remaining - $seconds);
            }
            $row->save();

            $usage = VoiceProviderUsage::query()->firstOrCreate(
                ['voice_provider_account_id' => $row->id, 'usage_date' => now()->toDateString()],
                ['audio_seconds' => 0, 'requests' => 0, 'successful_requests' => 0, 'failed_requests' => 0, 'input_bytes' => 0, 'output_bytes' => 0]
            );
            $usage->increment('audio_seconds', $seconds);
            $usage->increment('input_bytes', max(0, $inputBytes));
            $usage->increment('output_bytes', max(0, $outputBytes));
        });
    }
    ...
    public function recordRequest(VoiceProviderAccount $account, bool $success, ?int $latencyMs = null): void
    {
        $updates = [
            ...
            'last_failure_at' => $success ? $account->last_failure_at : now(),
            'last_latency_ms' => $latencyMs,
            'consecutive_failures' => $success ? 0 : ((int) $account->consecutive_failures + 1),
        ];
        if (!$success) {
            $failures = (int) $account->consecutive_failures + 1;
            $updates['cooldown_until'] = now()->addSeconds(min(300, 5 * (2 ** min(6, $failures - 1))));
            if ($failures >= 8) $updates['healthy'] = false;
        } else {
    ...
    public function resetMonthlyPeriodIfNeeded(VoiceProviderAccount $account): void
    {
        ...
        $account->quota_period_started_at = $start;
        $account->quota_period_ends_at = $end;
        $account->save();
    }
}
