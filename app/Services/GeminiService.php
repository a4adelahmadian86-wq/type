...
۱۰. پاسخ باید فقط JSON مطابق schema باشد.
...
PROMPT;
        ...
        $interaction = AiInteraction::create([
            ...
            'provider' => 'gemini',
            'model' => $model,
            'operation' => 'ocr_transcription',
            'request_id' => $requestId,
            'source_hash' => $context['source_hash'] ?? null,
            'prompt_hash' => hash('sha256', $prompt),
            'input_bytes' => (int) ($context['input_bytes'] ?? 0),
            'status' => 'started',
            'input_meta' => ['mime' => $mime, 'source_name' => $context['source_name'] ?? null],
        ]);
        ...
        try {
            ...
            if ($mime === 'application/zip') {
                ...
                $tmp = tempnam(sys_get_temp_dir(), 'farast_zip_');
                file_put_contents($tmp, $raw);
                $zip = new ZipArchive();
                abort_unless($zip->open($tmp) === true, 422, 'فایل ZIP قابل خواندن نیست');
                $allowed = ['jpg' => 'image/jpeg', 'jpeg' => 'image/jpeg', 'png' => 'image/png', 'webp' => 'image/webp'];
                for ($i = 0; $i < $zip->numFiles && $imageCount < 40; $i++) {
                    $name = $zip->getNameIndex($i);
                    $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));
                    if (!isset($allowed[$ext])) continue;
                    $data = $zip->getFromIndex($i);
                    if ($data === false) continue;
                    $inputs[] = ['type' => 'image', 'data' => base64_encode($data), 'mime_type' => $allowed[$ext]];
                    $imageCount++;
                }
                $zip->close();
                @unlink($tmp);
                abort_if($imageCount === 0, 422, 'فایل ZIP شامل تصویر قابل پردازش نیست');
            } elseif (str_starts_with($mime, 'image/')) {
                $inputs[] = ['type' => 'image', 'data' => $base64, 'mime_type' => $mime];
                $imageCount = 1;
            } elseif ($mime === 'application/pdf') {
                $inputs[] = ['type' => 'document', 'data' => $base64, 'mime_type' => 'application/pdf'];
            } else {
                abort(422, 'نوع فایل برای OCR پشتیبانی نمی‌شود');
            }
            ...
            $response = Http::timeout(180)
                ...
                ->post(self::ENDPOINT, [
                    ...
                    'input' => $inputs,
                    'store' => false,
                    'system_instruction' => 'دقت رونویسی از هر نوع زیباتر کردن متن مهم‌تر است. متن را هرگز از خودت تولید نکن.',
                            ...
                            'properties' => [
                                ...
                                'blocks' => [
                                    ...
                                    'items' => [
                                        'type' => 'object',
                                        'properties' => [
                                            'type' => ['type' => 'string', 'enum' => ['paragraph','heading','list_item','quote','blank']],
                                            'level' => ['type' => 'integer'],
                                            'text' => ['type' => 'string'],
                                            'uncertain' => [
                                                'type' => 'array',
                                        ...
                                        'required' => ['type','level','text','uncertain'],
                                    ],
                                ],
                            ],
                            'required' => ['rejected','reason','page_count','blocks'],
                        ],
                    ],
                ]);
            ...
            $rawOutput = collect($response->json('outputs', []))
                ->filter(fn ($o) => ($o['type'] ?? null) === 'text')
                ->pluck('text')->implode('');
            if ($rawOutput === '') {
                $rawOutput = collect($response->json('steps', []))
                    ->flatMap(fn ($s) => $s['content'] ?? [])
                    ->filter(fn ($c) => ($c['type'] ?? null) === 'text')
                    ->pluck('text')->implode('');
            }
            $json = json_decode($rawOutput, true);
            if (!is_array($json)) throw new \RuntimeException('Gemini returned invalid JSON');

            $interaction->update([
                'provider_interaction_id' => $providerId,
                'latency_ms' => $latency,
                'output_bytes' => strlen($rawOutput),
                'status' => 'completed',
                'output_meta' => [
                    'http_status' => $response->status(),
                    'image_count' => $imageCount,
                    'block_count' => count($json['blocks'] ?? []),
                    'rejected' => (bool) ($json['rejected'] ?? false),
                ],
            ]);

            Log::info('farast.ai.completed', [
                'request_id' => $requestId,
            ...
            $json['_ai_interaction_id'] = $interaction->id;
            $json['_request_id'] = $requestId;
            return $json;
        } catch (\Throwable $e) {
            $latency = (int) ((hrtime(true) - $started) / 1_000_000);
            $interaction->update([
