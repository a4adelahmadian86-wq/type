...
class AdminController extends Controller
{
    public function index(CapabilityService $capabilities)
    {
        ...
        return view('admin.index', [
            ...
            'stats' => ['users' => User::count(), 'documents' => \App\Models\TypingDocument::count(), 'ai' => AiInteraction::count(), 'ai_today' => AiInteraction::whereDate('created_at', today())->count()],
            'settings' => $settings,
            'secretStatus' => ['gemini' => (bool) (SiteSetting::read('gemini_api_key') ?: config('services.gemini.key')), 'files' => (bool) (SiteSetting::read('files_api_key') ?: env('FILES_API_KEY'))],
    ...
    {
        $data = $r->validate(['weekly_free_pages'=>['nullable','integer','min:0','max:999999'],'max_file_mb'=>['nullable','integer','min:1','max:2048'],'daily_ai_requests'=>['nullable','integer','min:0','max:999999'],'gemini_model'=>['nullable','string','max:120'],'can_type'=>['nullable','boolean'],'can_ai'=>['nullable','boolean'],'can_voice'=>['nullable','boolean'],'can_export_docx'=>['nullable','boolean'],'can_export_pdf'=>['nullable','boolean'],'can_feedback'=>['nullable','boolean'],'can_support'=>['nullable','boolean'],'gemini_api_key'=>['nullable','string','max:500'],'files_api_key'=>['nullable','string','max:500']]);
        foreach (['weekly_free_pages','max_file_mb','daily_ai_requests','gemini_model'] as $key) if (array_key_exists($key, $data) && $data[$key] !== null) SiteSetting::write($key, $data[$key]);
