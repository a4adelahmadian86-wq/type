...
class ExportController extends Controller
{
    public function export(Request $r, string $format)
    {
        ...
        $canvas=$pdf->getCanvas();$canvas->page_script(function($pageNumber,$pageCount,$canvas,$fontMetrics){$margin=28.8;$canvas->rectangle($margin,$margin,$canvas->get_width()-($margin*2),$canvas->get_height()-($margin*2),[0.85,0.87,0.90],0.7);});
        return response($pdf->output(),200,['Content-Type'=>'application/pdf','Content-Disposition'=>'attachment; filename="farast-'.$doc->id.'.pdf"']);
    }

    private function docx(TypingDocument $doc,string $content)
    {
        $w=new PhpWord();$w->setDefaultFontName('B Nazanin');$w->setDefaultFontSize(16);$w->setDefaultParagraphStyle(['lineSpacing'=>276,'spaceAfter'=>160,'alignment'=>'right']);
        $s=$w->addSection(['paperSize'=>'A4','marginTop'=>1440,'marginBottom'=>1440,'marginLeft'=>1440,'marginRight'=>1440,'borderSize'=>4,'borderColor'=>'D9DDE3','borderStyle'=>'single']);
        PhpWordHtml::addHtml($s,$content,false,false);
    ...
    {
        $html=preg_replace('/<span\b[^>]*class=(?:"[^"]*\bai-uncertain\b[^"]*"|\'[^\']*\bai-uncertain\b[^\']*\')[^>]*>(.*?)<\/span>/isu','$1',$html)??$html;
        $html=preg_replace('/\sdata-(?:original|suggestions)=(?:"[^"]*"|\'[^\']*\')/iu','',$html)??$html;
        return $html;
    }
}
