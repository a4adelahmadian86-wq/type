...
class PaymentController extends Controller
{
    ...
    public function payWithWallet(Request $request, Order $order, PricingService $pricing, FreeQuotaService $free)
    {
        ...
        $request->validate(['accept_terms'=>['accepted']]);
        try {
            DB::transaction(function () use ($order, $pricing, $free) {
                ...
                [$subtotal,$discount,$tax,$total] = $this->totals($quote,$freeApplied);
                $hash = hash('sha256',(string)$document->content);
                $locked->update(['subtotal_rials'=>$subtotal,'discount_rials'=>$discount,'tax_rials'=>$tax,'total_rials'=>$total,'free_pages_applied'=>$freeApplied,'content_hash'=>$hash,'pricing_snapshot'=>['content_hash'=>$hash,'quote'=>$quote,'tax_rate'=>$this->taxRate(),'free_pages_available'=>$freeAvailable],'terms_accepted_at'=>now()]);
                ...
                $before=(int)$wallet->balance_rials; $after=$before-$total;
                if($total>0){$wallet->update(['balance_rials'=>$after]);WalletTransaction::create(['wallet_id'=>$wallet->id,'type'=>'debit','amount_rials'=>$total,'balance_before'=>$before,'balance_after'=>$after,'reference_type'=>'order','reference_id'=>$locked->id,'description'=>'پرداخت هزینه خروجی سند','idempotency_key'=>'order-'.$locked->id]);}
                ...
                $locked->update(['status'=>'paid','paid_at'=>now()]); $document->update(['status'=>'paid','price_rials'=>$total]);
            });
    ...
    private function totals(array $quote,int $freeApplied): array { $subtotal=max(0,(int)$quote['price_rials']);$discount=min($subtotal,max(0,$freeApplied*(int)$quote['breakdown']['base_page']));$tax=$this->tax($subtotal-$discount);return[$subtotal,$discount,$tax,max(0,$subtotal-$discount+$tax)]; }
