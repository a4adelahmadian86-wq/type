...
class AuthController extends Controller
{
    ...
    {
        $phone = $request->session()->get('register.mobile');
        $verified = $request->session()->boolean('register.verified');
        if (!$phone) return redirect()->route('login');
        return view('auth.register', compact('phone', 'verified'));
    }
    ...
    public function verifyEmailOtp(Request $request)
    {
        ...
        if ($otp->attempts >= 5) abort(429, 'تعداد تلاش‌ها بیش از حد مجاز است.');
    ...
    public function emailMobileForm(Request $request)
    {
        ...
        if (!$user) return redirect()->route('login.email');
        return view('auth.email-mobile', ['mobile' => $user->mobile]);
    }
    ...
    {
        return view('auth.forgot-password', ['step' => 'phone']);
    }
    ...
    public function sendPasswordResetOtp(Request $request, SmsService $sms)
    {
        $phone = $this->validatedPhone($request);
        $user = User::where('mobile', $phone)->first();
        if (!$user) throw ValidationException::withMessages(['mobile' => 'حسابی با این شماره پیدا نشد.']);
        $request->session()->put('password_reset.mobile', $phone);
        $this->issueSmsOtp($phone, $sms);
        return response()->json(['ok' => true, 'next' => route('password.request', ['step' => 'otp']), 'message' => 'کد بازیابی برای شما پیامک شد.']);
    }
    ...
    {
        abort_unless($request->session()->boolean('password_reset.verified'), 403);
    ...
    {
        $phone = $request->session()->get('password_reset.mobile');
        abort_unless($phone && $request->session()->boolean('password_reset.verified'), 403);
        ...
        Auth::login($user, true);
        $request->session()->regenerate();
        return redirect()->intended(route('editor'))->with('status', 'رمز عبور با موفقیت تغییر کرد.');
    }
    ...
    {
        $phone = $this->normalizePhone((string) $request->input('mobile'));
        validator(['mobile' => $phone], ['mobile' => ['required', 'regex:/^09\d{9}$/']], ['mobile.regex' => 'شماره موبایل معتبر نیست.'])->validate();
        return $phone;
    }
